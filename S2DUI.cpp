...
namespace S2DUI
{
	...
	void renderFancyBox(S2DGL::Renderer2D& renderer, glm::vec4 transform, glm::vec4 color, S2DGL::Texture t, bool hovered, bool clicked)
	{
		...
		else
		{
			...
			//renderer.render9Patch2(transform, newColor, {}, 0.f, t, S2DGL_DefaultTextureCoords, {2.f/26.f, 24.f / 26.f,24.f / 26.f,2.f / 26.f});
			renderer.render9Patch2(transform, newColor, {}, 0.f, t, S2DGL_DefaultTextureCoords, {0.2,0.8,0.8,0.2});
		
		}
	}

	//just z and w components of transform used
	//todo: move into S2DGL a function to render text of a size
	float determineTextSize(S2DGL::Renderer2D &renderer, const std::string &str, S2DGL::Font &f, glm::vec4 transform, bool minimize = true)
	{
	...
	glm::vec4 determineTextPos(S2DGL::Renderer2D& renderer, const std::string& str, S2DGL::Font& f, glm::vec4 transform,
	...
	void renderText(S2DGL::Renderer2D& renderer,const std::string &str, S2DGL::Font& f, glm::vec4 transform, glm::vec4 color, 
		bool noTexture, bool minimize = true)
	{
		auto newStr = getString(str);
	...
	glm::vec4 computeTextureNewPosition(glm::vec4 transform, S2DGL::Texture t)
	{
	...
	void renderTexture(S2DGL::Renderer2D &renderer, glm::vec4 transform, S2DGL::Texture t, S2DGL::Color4f c, glm::vec4 textureCoordonates)
	{
	...
	void renderSliderFloat(S2DGL::Renderer2D &renderer, glm::vec4 transform, float *value, float min, float max, 
		bool &sliderBeingDragged,
		S2DGL::Texture barT, S2DGL::Color4f barC, S2DGL::Texture ballT, S2DGL::Color4f ballC, RendererUi::Internal::InputData &input)
	{
	...
	void renderSliderInt(S2DGL::Renderer2D &renderer, glm::vec4 transform, int *value, int min, int max,
		bool &sliderBeingDragged,
		S2DGL::Texture barT, S2DGL::Color4f barC, S2DGL::Texture ballT, S2DGL::Color4f ballC, RendererUi::Internal::InputData &input)
	{
	...
	void RendererUi::renderFrame(S2DGL::Renderer2D& renderer, S2DGL::Font& font, glm::ivec2 mousePos, bool mouseClick,
		bool mouseHeld, bool mouseReleased, bool escapeReleased, const std::string& typedInput, float deltaTime)
	{
		...
		{
			if (widgetsCopy[i].second.type == S2DUI::widgetType::newColumW)
			{
				float padd = 0;
	...
	bool RendererUi::Button(std::string name, const S2DGL::Color4f colors, const S2DGL::Texture texture)
	{
		name += internal.idStr;
	...
	void RendererUi::Texture(int id, S2DGL::Texture t, S2DGL::Color4f colors, glm::vec4 textureCoords)
	{
		Internal::Widget widget = {};
	...
	bool RendererUi::ButtonWithTexture(int id, S2DGL::Texture t, S2DGL::Color4f colors, glm::vec4 textureCoords)
	{
		Internal::Widget widget = {};
	...
	bool RendererUi::Toggle(std::string name, const S2DGL::Color4f colors, bool* toggle, const S2DGL::Texture texture, const S2DGL::Texture overTexture)
	{
		name += internal.idStr;
	...
	void RendererUi::Text(std::string name, const S2DGL::Color4f colors)
	{
		name += internal.idStr;
	...
	void RendererUi::InputText(std::string name, char* text, size_t textSizeWithNullChar,
		S2DGL::Color4f color, const S2DGL::Texture texture)
	{
		name += internal.idStr;
	...
	void RendererUi::sliderFloat(std::string name, float *value, float min, float max,
		S2DGL::Texture sliderTexture, S2DGL::Color4f sliderColor,
		S2DGL::Texture ballTexture, S2DGL::Color4f ballColor)
	{
		name += internal.idStr;
	...
	void RendererUi::sliderInt(std::string name, int *value, int min, int max,
		S2DGL::Texture sliderTexture, S2DGL::Color4f sliderColor, S2DGL::Texture ballTexture, S2DGL::Color4f ballColor)
	{
		name += internal.idStr;
	...
	void RendererUi::colorPicker(std::string name, float *color3Component, S2DGL::Texture sliderTexture, S2DGL::Texture ballTexture)
	{
		Internal::Widget widget = {};
	...
	void RendererUi::BeginMenu(std::string name, const S2DGL::Color4f colors, const S2DGL::Texture texture)
	{
		Internal::Widget widget = {};
